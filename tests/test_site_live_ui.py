...
class SiteLiveUiTest(unittest.TestCase):
    ...
    def test_volunteer_field_console_page_is_real_app_experience(self):
        ...
        self.assertIn("Voice note is local only in this version", app)
        ...
        self.assertIn("Images and text notes", app)
        ...
        self.assertIn("formData.append('image', file);", js)
        self.assertIn("formData.append('note', report);", js)
        self.assertNotIn("formData.append('audio'", js)
        self.assertNotIn("formData.append('field-audio'", js)
        self.assertNotIn("formData.append('voice'", js)
        self.assertNotIn("X-Judge-Token", js)
        self.assertNotIn("edge-triage-judge-token", js)
        self.assertNotIn("Paste the judge token", js)
        self.assertNotIn("renderFieldResult(bridgeSample, report);", js)
        self.assertIn("clearBridgeDefaultsForNewInput", js)
        self.assertIn("New field report", js)
        self.assertIn("Awaiting analysis", js)
        self.assertIn("No triage result yet", app)
        self.assertNotIn("Bridge washed out after flood. Road blocked. No injuries visible. Need routing guidance.", app)
        self.assertNotIn("Blocked bridge report", app)
    ...
    def test_judge_guide_and_trust_notes_help_different_reviewers(self):
        ...
        self.assertIn("Live preview: guarded real Gemma API", html)
